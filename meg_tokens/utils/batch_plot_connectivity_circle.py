"""Group-level circular plots for Stage 10 connectivity derivatives."""
...
def _load_old_connectivity_pairs(data_dir: str, condition: str, band: str) -> tuple[np.ndarray, np.ndarray]:
    base = Path(data_dir)
    before_files = sorted(base.glob(f"**/*_{condition}_{band}_con_before_ROI.npy"))
    if not before_files:
        raise FileNotFoundError(f"No connectivity matrices found for condition={condition}, band={band}")

    before = []
    after = []
    for before_file in before_files:
        after_file = Path(str(before_file).replace("_con_before_ROI.npy", "_con_after_ROI.npy"))
        if not after_file.is_file():
            raise FileNotFoundError(f"Missing after-window matrix paired with {before_file}: {after_file}")
        before.append(load_array(before_file, expected_ndim=2).data)
        after.append(load_array(after_file, expected_ndim=2).data)
    return np.stack(before, axis=0), np.stack(after, axis=0)
...
def load_connectivity_pairs_with_metadata(
    ...
    if not modern_rows:
        before, after = _load_old_connectivity_pairs(data_dir, condition, band)
        node_names = [f"node_{idx:03d}" for idx in range(before.shape[-1])]
        subjects = [f"sample_{idx + 1:03d}" for idx in range(before.shape[0])]
        return before, after, node_names, subjects, []
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Plot circular functional connectivity from Stage 10 derivatives.")
    parser.add_argument("--data_dir", type=str, required=True,
                        help="Directory containing Stage 10 connectivity derivatives.")
    parser.add_argument("--out_dir", type=str, required=True,
                        help="Directory to save stats and plots.")
    parser.add_argument("--condition", type=str, required=True,
                        help="Condition name to plot.")
