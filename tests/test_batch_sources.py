...
from meg_tokens.utils.batch_sources import (
    find_epoch_file,
    find_noise_file,
    find_trans_file,
    model_paths,
...
def test_find_epoch_file_matches_stage2_contract(tmp_path):
    ...
    assert find_epoch_file(str(tmp_path), "H1", "Slow1", None, "go") == str(epoch_file)
...
def test_find_trans_file_matches_subject_and_run(tmp_path):
    ...
    assert find_trans_file(str(tmp_path), "H01", "1") == str(trans_file)


def test_model_paths_are_derivative_paths(tmp_path):
    paths = model_paths(str(tmp_path), "H1", "oct6")
...
def test_model_paths_keep_mixed_source_spaces_distinct(tmp_path):
    paths = model_paths(str(tmp_path), "H1", "oct6", volume_labels=["Left-Putamen"])
